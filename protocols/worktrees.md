...
2. **NEVER pass `isolation: "worktree"`** to the `Agent` tool for a pair/researcher, and tell every pair/researcher in its dispatch prompt: **never call `EnterWorktree`** — background subagents can call it themselves (CC 2.1.241) and it creates the same throwaway `.claude/worktrees/agent-*` worktree. Either path spawns a throwaway harness worktree (`.claude/worktrees/agent-<hash>`, lock reason `claude agent (pid …)`) that is NOT auto-cleaned when dirty — it accumulates and becomes stale-locked. The pool already provides isolation.
