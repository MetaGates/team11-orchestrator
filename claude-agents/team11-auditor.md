...
The guard is a tripwire against ACCIDENTAL mutation, not a sandbox: variable indirection (`X=rm; $X -rf …`) and pre-written script files run by an interpreter are out of scope by design — the audit contract, checkpoints, and commit gates are the real backstop. Do not treat a guard ALLOW as permission to mutate; read-only is the rule regardless of what the tripwire happens to catch.
